...
#EDD attention function:
def distance_decay_factor_matrix(end_x:int,end_y:int,include_cls:bool=True):
    t_x,t_y =  init_t_xy(end_x,end_y)
    coords = torch.stack([t_x,t_y],dim=-1)
    dist = torch.cdist(coords, coords, p=2)        
    if include_cls:
        Np = dist.shape[0]
        full = torch.zeros(Np + 1, Np + 1, dtype=dist.dtype)
        full[1:, 1:] = dist
        return full
    return dist
...
class EDDAttention(Attention):
    def __init__(self, *args, edd_num_scales: int = 4,
                 edd_init_decays=(0.5, 1.0, 2.0, 4.0), **kwargs):
        """
        args:
            edd_num_scales: The number of different base decay rates the models can choose from (K)
            edd_init_decays: List of base decays rates -> shape(edd_init_decays) = (K,) 
        """
        super().__init__(*args, **kwargs)
        assert len(edd_init_decays) == edd_num_scales
        init_decays = torch.tensor(edd_init_decays, dtype=torch.float32) #create tensor for decays
        inv_softplus = init_decays + torch.log(-torch.expm1(-init_decays))

        #Registers the inverse-softplus values as learnable weights. The model can now tweak these 4 base scales during backpropagation.
        self.edd_log_decay = nn.Parameter(inv_softplus)                          # shape(edd_log_decay) = (K,)

        #Map num_heads to edd_num_scales and registers it as learnable weights
        self.edd_mix_logits = nn.Parameter(torch.zeros(self.num_heads, edd_num_scales))  # (H,K)
        self.edd_gate = nn.Parameter(torch.zeros(self.num_heads))                # (H,)

    def forward(self, x, freqs_cis, dist):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]
        ...
        attn = (q * self.scale) @ k.transpose(-2, -1)     # <- identical to RoPEAttention up to here

        decay = F.softplus(self.edd_log_decay)              # (K,) > 0
        mix = self.edd_mix_logits.softmax(dim=-1)             # (H,K), convex combination
        head_decay = (mix * decay.unsqueeze(0)).sum(-1)       # (H,)
        gate = torch.sigmoid(self.edd_gate)                   # (H,)
        rate = (gate * head_decay).view(1, self.num_heads, 1, 1)
        attn = attn + (-rate * dist.view(1, 1, N, N)).to(attn.dtype)   # <- the one new line
...
class RoPE_Layer_scale_init_Block(Layer_scale_init_Block):
    ...
    def forward(self, x, freqs_cis, dist=None):
        # dist is accepted (and ignored) so this block is interchangeable with
        # EDD_Layer_scale_init_Block from the caller's point of view -
        # forward_features() calls every block with dist=dist regardless of type.
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), freqs_cis=freqs_cis))
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        
        return x

class EDD_Layer_scale_init_Block(Layer_scale_init_Block):
    def __init__(self, *args, **kwargs):
        kwargs["Attention_block"] = EDDAttention
        super().__init__(*args, **kwargs)

    def forward(self, x, freqs_cis, dist):
        # BUG FIX: dist was computed but never forwarded to self.attn(), so
        # EDDAttention.forward() (which requires dist) raised a TypeError.
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), freqs_cis=freqs_cis, dist=dist))
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
...
class rope_vit_models(vit_models):
    def __init__(self, rope_theta=100.0, rope_mixed=False, use_ape=False,
        ...
        # BUG FIX: edd_dist used to only be registered inside the rope_mixed
        # branch below, but forward_features() reads self.edd_dist
        # unconditionally - every axial (rope_mixed=False) model raised
        # AttributeError. Register it here so it exists for both modes.
        grid = img_size // patch_size
        self.register_buffer('edd_dist', distance_decay_factor_matrix(grid, grid, include_cls=True))
    ...
    def forward_features(self, x):
        ...
        if self.rope_mixed:
            ...
            #Added
            if self.edd_dist.shape[0] != x.shape[1]:
                dist = distance_decay_factor_matrix(W // self.patch_size, H // self.patch_size, include_cls=True).to(x.device)
            else:
                dist = self.edd_dist.to(x.device)
            for i , blk in enumerate(self.blocks):
                x = blk(x, freqs_cis=freqs_cis[i],dist=dist)
        else:
            ...
            if self.edd_dist.shape[0] != x.shape[1]:
                ...
                dist = self.edd_dist.to(x.device)
            for i , blk in enumerate(self.blocks):
                x = blk(x, freqs_cis=freqs_cis,dist=dist)
...
@register_model
def edd_rope_mixed_deit_small_patch8_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=8, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        block_layers=EDD_Layer_scale_init_Block, Attention_block=EDDAttention,
        rope_theta=10.0, rope_mixed=True, **kwargs)
    model.default_cfg = _cfg()
    return model