...
def load_model(model_path=None):
    ...
    TARGET_MAX_PIXELS = 1_048_576
    ALIGNED_PIXELS = (TARGET_MAX_PIXELS // 1024) * 1024
    print("Aligned pixels:",ALIGNED_PIXELS)
    tokenizer.image_processor.size.longest_edge = ALIGNED_PIXELS
    tokenizer.image_processor.size.shortest_edge = 65536
    #print(type(tokenizer.image_processor))
    #print(vars(tokenizer.image_processor))
    print(tokenizer.image_processor.size)
...
def build_grpo_config():
    training_args = GRPOConfig(
        #max_steps=5000,   #Only use when no need of full runs
...
def main():
    ...
    train_dataset = load_robo2vlm(dataset_name=DataConfig.DATASET_NAME,system_prompt=SYSTEM_PROMPT,tokenizer=tokenizer,split="train")
    ...
    test_dataset = load_robo2vlm(dataset_name=DataConfig.DATASET_NAME,system_prompt=SYSTEM_PROMPT,tokenizer=tokenizer,split="test")
    sample = train_dataset[0]
